...
public class MyListener implements ServletContextListener{
	...
	try{
		...
		con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL","KINSHUK","MANTU");
		...
		PreparedStatement ps2= con.prepareStatement("CREATE SEQUENCE NACRE MINVALUE 1 MAXVALUE 999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE");
		ps2.executeUpdate();
		...
		stmt.executeUpdate("CREATE TRIGGER  BI_QUIZINFO before insert on QUIZINFO for each row begin select NACRE.nextval into :NEW.QUIZNAME from dual;end");
		stmt.executeUpdate("CREATE TRIGGER  BI_QUIZQUES before insert on QUIZQUES for each row begin select NACRE.nextval into :NEW.QID from dual;end");
		}	
	}
